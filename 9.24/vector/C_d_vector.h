...
// Add Part(9_26_2015):
		...
		friend ostream &operator<< (ostream&, const miniVector<T1>&);
			// assignment operator.
			// Postcondition: output current vector data.

		template <typename T1>
		friend istream &operator>> (istream& , miniVector<T1>&);
			// assignment operator.
			// Postcondition: input vector data.

		void insert(const int place, const T& item);
			// insert an item at a certain place of the vector.
			// Postcondition: the vector size is increased by 1.

		void cut(const int place, const T& item);
			// delete an item at a certain place of the vector.
			// Postcondition: there exist an element equals the item.
...
// assignment operator.
template <typename T1>
ostream &operator<< (ostream& out, const miniVector<T1>& v)
{
	int n = v.size();
	for (int i = 0; i < n; i++)
		out << v.vArr[i] << "  ";
...
// assignment operator.
template <typename T1>
istream &operator>> (istream& in, miniVector<T1>& v)
{
	T1 temp;
	do {
		in >> temp;
...
// insert an item at some place of the vector.
template <typename T>
void miniVector<T>::insert(const int place, const T& item)
{
	if (vSize == vCapacity){
		if (vCapacity == 0){
			...
			reserve(1, false);
			vArr[0] = item;	
			// the place is meanless here.
		}
		
		else{
			reserve(2 * vCapacity, true);
			// if capacity is full,double the capacity.
			for (int i = vSize; i > place; i--){
				vArr[i] = vArr[i - 1];
			}
			// move every element after the insert place one 
			// address behind.

			vArr[place] = item;
			// after the movement, insert the place number.
		}	
	}

	else{
		// if vSize < vCapacity , the room is big enough.
		// just move the elements and insert the needed one.
		for (int i = vSize; i > place; i--){
			vArr[i] = vArr[i - 1];
		}
...
// delete an item at a certain place of the vector.
template <typename T>
void miniVector<T>::cut(const int place, const T& item)
{
	// if inserted place is larger than vSize or the element 
	// in the place is not equals to the item, output error.
	if (place>vSize || vArr[place] != item){
		cout << "Find error!"<<endl;
		return;
	}

	// just let every element after the place move one pace forward 
	// to cover the original data.
	else
		for (int i = place; i < vSize; i++)
			vArr[i] = vArr[i + 1];
	// update vSize.
	vSize--;
}
